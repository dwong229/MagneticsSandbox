function varargout = Fonaxis_2coilslowhysteresisGUI(varargin)
...
gradBTotal = gradBLeft + gradBRight;
% compute F
FLeft = computeFelectromagnet(gradBTotal(magnetIdx(1)),handles.Mx*sign(gradBTotal(magnetIdx(1))));
FRight = computeFelectromagnet(gradBTotal(magnetIdx(2)),handles.Mx*sign(gradBTotal(magnetIdx(2))));
%% end from Fonaxis_2coils

%h_Force(1) = 
axes(handles.axes1)
title('Force profile')
handles.FLeft = plot(handles.x,gradBLeft,'b');
hold on
handles.FRight = plot(handles.x,gradBRight,'r');
handles.FTotal = plot(handles.x,gradBTotal,'.k');
xlabel('Position')
ylabel('dB_x/dx')
...
%% subplot(2,1,2)
% print zero crossing
zeroCrossExists = checkZeroCross(gradBTotal);
